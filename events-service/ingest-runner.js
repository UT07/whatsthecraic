...
(async () => {
  ...
  try {
    ...
    if (BANDSINTOWN_APP_ID && BANDSINTOWN_SEED_ARTISTS.length) {
      const artists = BANDSINTOWN_ALLOW_ANY_ARTIST ? BANDSINTOWN_SEED_ARTISTS : BANDSINTOWN_SEED_ARTISTS;
      results.push(await ingestBandsintownArtists(pool, {
        artists: artists.slice(0, BANDSINTOWN_MAX_ARTISTS),
    ...
    results.push(await ingestDiceApify(pool, {
      city,
      startDate: start,
      endDate: end,
